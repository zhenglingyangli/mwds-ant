...
int *pool_sols[POOL_MAX_SIZE];
long pool_costs[POOL_MAX_SIZE];
int pool_size = 0;
int pool_freq[POOL_MAX_SIZE];
int *vertex_weight;
...
int build_instance_massive(char *filename)
{
    ...
    for(int pi = 0; pi < POOL_MAX_SIZE; pi++){
      pool_sols[pi] = (int *)malloc(vertex_num*sizeof(int));
      memset(pool_sols[pi], 0, vertex_num*sizeof(int));
      pool_costs[pi] = LONG_MAX;
      pool_freq[pi] = 0;
    }
    pool_size = 0;
  ...
  for(j=0;j<vertex_num;j++){
    ...
    undom_freq = (int *)calloc(vertex_num, sizeof(int));
    undom_freq_avg = 0;
