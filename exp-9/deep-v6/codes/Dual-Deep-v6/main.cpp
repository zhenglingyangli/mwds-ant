...
int main(int argc, char* argv[])
{
  ...
  /*
   * exp-9: keep AQ feedback available on dense structured graphs as well.
   * exp-8 disabled USE_ADVANCED when density > 15, which leaves BHOSLIB/DIMACS
   * with almost no learned feedback despite exactly being the cases where the
   * initial LB often stalls below UB.
   */
  USE_ADVANCED = (NB_NODE >= 800 || density >= 50.0) ? 1 : 0;
  printf("[v6] |V|=%d |E|=%d density=%.2f USE_ADVANCED=%d\n",
  ...
  while(1){
    ...
    if(!ibmwds_init_bounds(i++,&timeout)){
	 ...
	 if(i==1){
	   printf("\n Switch to LB-first search + AQ boost, timeout %lf....\n",timeout);
	   AQ_ENABLED = 1;
	   double rl_deadline;
	   if(FIRST_GAP < 0.01 && density <= 1.0){
	     printf("\n [Tier-1 Easy] gap=%.4lf density=%.2lf |V|=%d => No LB warmup\n",
		    FIRST_GAP, density, NB_NODE);
	     rl_deadline = 0;
	   }else if(FIRST_GAP <= 0.5 && density <= 10.0){
	     printf("\n [Tier-2 Medium] gap=%.4lf density=%.2lf |V|=%d => Standard LB warmup (K=%d, 15%%)\n",
		    FIRST_GAP, density, NB_NODE, AQ_NUM_ANTS);
	     rl_deadline = time_Cutoff * 0.15;
	   }else{
	     if(AQ_NUM_ANTS < 8) AQ_NUM_ANTS = 8;
	     AQ_RHO = 0.08f;
	     printf("\n [Tier-3 Hard] gap=%.4lf density=%.2lf |V|=%d => Heavy LB warmup (K=%d, 25%%)\n",
		    FIRST_GAP, density, NB_NODE, AQ_NUM_ANTS);
	     rl_deadline = time_Cutoff * 0.25;
	   }
	   WeightSum rl_prev_lb = BEST_BEST_LOWER_BOUND;
	   int rl_stag = 0;
	   while(rl_deadline > 0 && getTimeElapsed() < rl_deadline){
	     round_time_Start=chrono::steady_clock::now();
	     reset_config();
	     if(ibmwds_init_bounds(i++,&timeout)){
	       break;
	     ...
	     if(rl_stag >= 20 && getTimeElapsed() > rl_deadline * 0.5){
	       printf(" [LB warmup exit] stag=%d at %.1f%%\n",
	 ...
	 /*
	  * exp-9: do not spend all remaining time in UB local search after the
	  * first LB warmup. Dense instances need repeated LB/UB alternation; a
	  * bounded UB slice preserves time for future ibmwds_init_bounds rounds.
	  */
	 double remaining = time_Cutoff - getTimeElapsed();
	 if(remaining <= 0) break;
	 double ub_slice = timeout;
	 double max_slice = remaining * 0.05;
	 if(max_slice < 1.0 && remaining > 1.0) max_slice = 1.0;
	 if(max_slice <= 0 || max_slice > remaining) max_slice = remaining;
	 if(ub_slice <= 0 || ub_slice > max_slice) ub_slice = max_slice;
	 timeout = ub_slice;
	 constructInit();
       }
      #ifndef NMLS
