...
static int compute_loss_and_upper_bound(WeightSum lb){
  ...
  /*
   * Only a tight LB/UB equality should terminate the bound search as OPTIMAL.
   *
   * The old condition `(rep_flag == 0 && loss_sum == 0)` is too aggressive for
   * exp-8 style LB tightening: on dense/structured instances (BHOSLIB, UDG,
   * DIMACS) the constructed ordering can have no replacement/loss while the
   * lower bound is still strictly below the best upper bound. Treating that
   * state as OPTIMAL stops subsequent Beta/AQ rounds before they can try to
   * lift the LB.
   */
