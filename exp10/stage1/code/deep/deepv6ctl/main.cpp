...
int main(int argc, char* argv[])
{
  ...
  printf("[v6] |V|=%d |E|=%d density=%.2f USE_ADVANCED=%d SAFE_AQ_MIN_GAP=%.3f DEEP_AQ_RANGE=[%d, %.2f, %.2f] structure_blocks_aq=%d AQ_SLOPE_MARGIN=%.2f AQ_MAX_UB_DRIFT=%.2f\n",
  ...
  while(1){
    ...
    if(!ibmwds_init_bounds(i++,&timeout)){
	 ...
	 if(i==1){
	   ...
	   double rl_deadline = getTimeElapsed() + time_Cutoff * aq_probe_frac;
	   int rl_stag = 0;
