...
int main(int argc, char *argv[]){
  ...
  total_step=INT_MAX;
  printf("%s #seed %d #alpha %.3lf #timeout %.3lf K=%d RHO=%.2f Q0=%.2f BETA=%.1f AQ_MODE=%s SAFE_AQ_MIN_GAP=%.3f SAFE_AQ_MIN_NODES=%d AQ_SLOPE_MARGIN=%.2f AQ_MAX_UB_DRIFT=%.2f\n",
         argv[1], seed, ALPHA, timeout, AQ_NUM_ANTS, AQ_RHO, AQ_Q0, AQ_BETA,
         aq_mode ? aq_mode : "auto", aq_min_first_gap, aq_min_nodes,
         aq_slope_margin, aq_max_ub_drift);
  srand(seed);
  read_time=get_utime();
  ...
  while(1){
    ...
    if(force_dbs){
      ...
      if(aq_arm == 0 || aq_arm == 4){
        ...
        AQ_ACTIVE = 1;
        AQ_TOUCH_UB = 0;
        rl_deadline = (get_utime()-read_time) + time_limit * 0.25;
      }
      if(rl_deadline > 0){
