...
EXP9_ROOT=Path(__file__).resolve().parents[2]
MANIFEST=EXP9_ROOT/'dataset_manifest.json'; SELECTED_DIR=EXP9_ROOT/'selected_instances'
...
def main():
    ...
    for ds in datasets:
        ...
        for seed in seeds:
            ...
            content=f"""#!/bin/sh
#SBATCH --job-name=e9_{c['solver_name'].replace('-','')[:6]}_{ds[:2]}_s{seed}
