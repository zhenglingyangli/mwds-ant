#!/bin/bash
# Preflight and submit exp-8 on the cluster.
#
# Usage:
#   bash exp-8/run_on_hpc.sh              # preflight, smoke test, then ask before submit
#   bash exp-8/run_on_hpc.sh --no-submit  # only preflight + smoke + generate scripts
#   bash exp-8/run_on_hpc.sh --submit     # preflight + smoke + submit without prompt
...
if [ "${1:-}" = "--no-submit" ]; then
  ...
  echo "Usage: bash exp-8/run_on_hpc.sh [--no-submit|--submit]"
  exit 2
fi
...
python3 -m json.tool dataset_manifest.json >/tmp/exp8_dataset_manifest.checked.json
...
out = root / ".exp8_resolved_instances.tsv"
...
python3 - <<'PY' >/tmp/exp8_smoke_instances.env
from pathlib import Path

root = Path.cwd()
wanted = ["T1", "UDG", "BHOSLIB", "DIMACS"]
resolved = {}
for line in (root / ".exp8_resolved_instances.tsv").read_text().splitlines():
...
source /tmp/exp8_smoke_instances.env
...
if [ "$MODE" = "prompt" ]; then
  echo
  echo "Type RUN to start full exp-8 submission now."
  read -r answer
  if [ "$answer" != "RUN" ]; then
