...
int main(int argc, char *argv[]){
  ...
  const char *small_probe_env = getenv("MWDS_AQ_SMALL_PROBE_FRAC");
  const char *normal_probe_env = getenv("MWDS_AQ_NORMAL_PROBE_FRAC");
  const char *heavy_probe_env = getenv("MWDS_AQ_HEAVY_PROBE_FRAC");
  const char *heavy_ants_env = getenv("MWDS_AQ_HEAVY_ANTS");
  ...
  double aq_slope_margin = slope_margin_env ? atof(slope_margin_env) : 1.05;
  double aq_max_ub_drift = max_ub_drift_env ? atof(max_ub_drift_env) : 0.25;
  double aq_small_probe_frac = small_probe_env ? atof(small_probe_env) : 0.05;
  double aq_normal_probe_frac = normal_probe_env ? atof(normal_probe_env) : 0.15;
  double aq_heavy_probe_frac = heavy_probe_env ? atof(heavy_probe_env) : 0.20;
  int aq_heavy_ants = heavy_ants_env ? atoi(heavy_ants_env) : 6;

  total_step=INT_MAX;
  printf("%s #seed %d #alpha %.3lf #timeout %.3lf K=%d RHO=%.2f Q0=%.2f BETA=%.1f AQ_MODE=%s SAFE_AQ_MIN_GAP=%.3f SAFE_AQ_MIN_NODES=%d AQ_SLOPE_MARGIN=%.2f AQ_MAX_UB_DRIFT=%.2f AQ_HEAVY_FRAC=%.2f AQ_HEAVY_ANTS=%d\n",
         argv[1], seed, ALPHA, timeout, AQ_NUM_ANTS, AQ_RHO, AQ_Q0, AQ_BETA,
         aq_mode ? aq_mode : "auto", aq_min_first_gap, aq_min_nodes,
         aq_slope_margin, aq_max_ub_drift, aq_heavy_probe_frac, aq_heavy_ants);
  srand(seed);
  read_time=get_utime();
  ...
  while(1){
    ...
    if(force_dbs){
      ...
      double rl_deadline;
      int aq_arm = 0; /* 0=DBS, 1=LIGHT, 2=STANDARD, 3=HEAVY, 4=UB_FOCUS */
      if(FIRST_GAP < aq_min_first_gap || (density > 10.0 && FIRST_GAP < 0.05)){
        aq_arm = 4;
      } else if(NB_NODE < aq_min_nodes){
        aq_arm = 1;
      ...
      printf("\n [AQ bandit arm] arm=%d gap=%.4lf density=%.2lf |V|=%d\n",
             aq_arm, FIRST_GAP, density, NB_NODE);

      if(aq_arm == 0 || aq_arm == 4){
        printf(" [AQ bandit] %s => No RL\n", aq_arm == 4 ? "UB_FOCUS/DBS-safe" : "DBS");
        AQ_ENABLED = 0;
        AQ_ACTIVE = 0;
        AQ_TOUCH_UB = 0;
        rl_deadline = 0;
        ...
        if(AQ_NUM_ANTS > 3) AQ_NUM_ANTS = 3;
        printf("\n [AQ small-headroom probe] gap=%.4lf density=%.2lf |V|=%d => Light RL (K=%d, %.1f%%)\n",
               FIRST_GAP, density, NB_NODE, AQ_NUM_ANTS, aq_small_probe_frac * 100.0);
        AQ_ENABLED = 1;
        AQ_ACTIVE = 1;
        AQ_TOUCH_UB = 0;
        rl_deadline = (get_utime()-read_time) + time_limit * aq_small_probe_frac;
      } 
      else if(aq_arm == 2){
        ...
        AQ_ACTIVE = 1;
        AQ_TOUCH_UB = 0;
        rl_deadline = (get_utime()-read_time) + time_limit * aq_normal_probe_frac;
      } 
      else {
        int hard_K = (NB_NODE > 100000) ? 10 : aq_heavy_ants;
        AQ_NUM_ANTS = hard_K;
        AQ_RHO = 0.08f;
        printf("\n [Tier-3 Hard tuned] gap=%.4lf density=%.2lf |V|=%d => Heavy RL (K=%d, %.1f%%)\n",
               FIRST_GAP, density, NB_NODE, AQ_NUM_ANTS, aq_heavy_probe_frac * 100.0);
        AQ_ENABLED = 1;
        AQ_ACTIVE = 1;
        AQ_TOUCH_UB = 0;
        rl_deadline = (get_utime()-read_time) + time_limit * aq_heavy_probe_frac;
      }
      if(rl_deadline > 0){
        ...
        double aq_start_time = get_utime()-read_time;
        ...
        double aq_time = (get_utime()-read_time) - aq_start_time;
