...
CONFIGS={'deep-v6':{'solvers':{'deep-v6'},'family':'deep','prefix':'exp9_deep'},'fast-v19':{'solvers':{'fast-v19'},'family':'fast','prefix':'exp9_v19'}}
SUB=Path(__file__).resolve().parents[1].name; CFG=CONFIGS[SUB]; EXP9_ROOT=Path(__file__).resolve().parents[2]; BASELINE_INDEX=EXP9_ROOT/'baseline_index.csv'
...
def report(p,rows,agg):
    lines=[f"# exp-9 {CFG['family']} LB summary",'',f'- runs parsed: {len(rows)}',f'- instances parsed: {len(agg)}','']
    if agg:
        by=defaultdict(list)
