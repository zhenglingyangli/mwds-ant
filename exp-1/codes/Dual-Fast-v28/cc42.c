...
void init_solution_perturbed(double skip_rate){
  int cnt, kn;
  int i,j,k,l,v;
  int sr, ct;

  for(i=0;i<USED(VEC_SOLUTION);i++){
    int vv=ITEM(VEC_SOLUTION,i)-1;
    if(cs[vv].locked) continue;
    if(rand()/(RAND_MAX+1.0) < skip_rate) continue;
    add(vv, 1, 0);
  }
  while(uncover_num > 0){
    int uncover_v = uncover_vertex[rand() % uncover_num];
    sr = cs[uncover_v].score;
    ct = cs[uncover_v].cost;
    best_array[0] = uncover_v;
    cnt = 1;
    ...
    for(v=0;v<kn;v++){
      ...
      k=compare(sr,ct,cs[j].score, cs[j].cost);
      if(sr==INT_MIN||k<0){
        sr=cs[j].score; ct=cs[j].cost;
        best_array[0]=j; cnt=1;
      } else if(k==0){
        best_array[cnt++]=j;
      }
    }
...
int main(int argc, char *argv[]){
  ...
  if(argc<5){
    printf("USAGE: ./dual-fast-v28 instance cutoff seed alpha [K] [rho] [q0] [beta]\n");
    return 0;
  }
  ...
  printf("[v28-MR] |V|=%d |E|=%d density=%.2f USE_ADVANCED=%d\n", NB_NODE, NB_EDGE, density, USE_ADVANCED);
  ...
  while(1){
    ...
    if(USE_ADVANCED){
      ...
      if(!lb_converged && lb_stagnation >= 3 && elapsed_ratio > 0.4){
        ...
        printf("\n [LB Done] stag=%d elapsed=%.1f%% => %.1fs to UB (multi-restart)\n",
               lb_stagnation, elapsed_ratio*100, remaining);
        if(remaining > 0){
          double first_budget = remaining * 0.4;
          if(first_budget < 0.5) first_budget = remaining;
          init_solution();
          localsearch(first_budget);
          if(!check()){ printf("wrong answer \n"); break; }

          int n_restart = 0;
          while(1){
            double t_left = time_limit - (get_utime()-read_time);
            if(t_left < 0.3) break;
            n_restart++;
            reset_config();
            init_solution_perturbed(0.15);
            localsearch(t_left);
            if(!check()){ printf("wrong answer \n"); break; }
          }
          printf(" [Multi-restart] did %d extra restarts, best_UB=%ld\n",
                 n_restart, best_best_value);
