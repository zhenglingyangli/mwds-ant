...
void localsearch(double _timeout){
  ...
  int rand_n;
  int init_remove = 0;
  memset(undom_freq, 0, vertex_num * sizeof(int));
  undom_freq_avg = 0;
  times(&start);
  real_time=0;
        ...
        for(v = 0; v < uncover_length; v++)
        {
            ...
            if(sr==INT_MIN||k<0){
              ...
              if(cs[maxc].config < cs[j].config)
                maxc = j;
              else if(cs[maxc].config == cs[j].config){
                if(undom_freq[j] > undom_freq[maxc])
                  maxc = j;
                else if(undom_freq[j] == undom_freq[maxc]){
                  if(AQ_ENABLED && AQ[j+1] > AQ[maxc+1])
                    maxc = j;
...
static void pool_try_insert(long cost){
  int worst_idx = -1;
  long worst_cost = 0;
  for(int p = 0; p < pool_size; p++){
    int same = 1;
    for(int v = 0; v < remain_num && same; v++){
      int j = remain_vertex[v];
      if(pool_sols[p][j] != cs[j].is_in_c) same = 0;
    }
    if(same) return;
    if(pool_costs[p] > worst_cost){
      worst_cost = pool_costs[p]; worst_idx = p;
    }
  }
  int target;
  ...
  pool_costs[target] = cost;
  pool_freq[target] = 0;
  for(int v = 0; v < remain_num; v++){
    int j = remain_vertex[v];
    pool_sols[target][j] = cs[j].is_in_c;
  }
}

static void pool_inject_pheromone(){
  if(pool_size < 2 || !AQ_ENABLED) return;
  for(int p = 0; p < pool_size; p++){
    float w = 1.0f / (1.0f + (float)(pool_costs[p] - pool_costs[0]) / (float)INIT_UPPER_BOUND);
    for(int v = 0; v < remain_num; v++){
      int j = remain_vertex[v];
      if(pool_sols[p][j]){
        AQ[j+1] += 0.02f * w;
        if(AQ[j+1] > AQ_TAU_MAX) AQ[j+1] = AQ_TAU_MAX;
      }
    }
  }
}
...
int main(int argc, char *argv[]){
  ...
  if(argc<5){
    printf("USAGE: ./exp9-freq-pool instance cutoff seed alpha [K] [rho] [q0] [beta]\n");
    return 0;
  }
  ...
  total_step=INT_MAX;
  printf("%s #seed %d #alpha %.3lf #timeout %.3lf K=%d RHO=%.2f Q0=%.2f BETA=%.1f\n",
  ...
  printf("[Exp9-Freq-Pool] |V|=%d |E|=%d density=%.2f USE_ADVANCED=%d\n", NB_NODE, NB_EDGE, density, USE_ADVANCED);
  ...
  while(1){
    ...
    if(i==1 && FIRST_GAP >= gap_thd){
      double rl_deadline;
      if(FIRST_GAP < 0.01 && density <= 1.0){
        printf("\n [Tier-1 Easy] gap=%.4lf density=%.2lf |V|=%d => No RL\n",
