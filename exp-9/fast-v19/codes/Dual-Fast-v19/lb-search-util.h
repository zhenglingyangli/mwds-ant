...
static int compute_loss_and_upper_bound(WeightSum lb){
   ...
   * exp-9: A no-replacement/no-loss ordering is not enough evidence that the
   * lower bound cannot be improved by later Beta/AQ rounds; stopping here
   * caused dense/structured instances to leave LB unchanged after 1-2 rounds.
   */
...
static inline int compute_bounds(int k){
  ...
  do{
    ...
    for(int ant = 0; ant < cur_K; ant++){ //
      ...
      #else
      if(ant_lb > BestAnt_LB ||
         (ant_lb == BestAnt_LB && (ant_score > BestAnt_Score || BestAnt_PairLength == 0))){
      #endif
