...
int main(int argc, char *argv[]){
  ...
  double timeout=1.0;
  if(argc<5){
    printf("USAGE: ./dualcc2v3 instance cutoff seed alpha \n");
    return 0;
  }
...
if(vertex_num<10000)
 ...
 seed=atoi(argv[3]);
  ...
  total_step=INT_MAX;
  printf("%s #seed %d #alpha %.3lf #timeout %.3lf \n",argv[1], seed,ALPHA,timeout);
  srand(seed);
  read_time=get_utime();
  ...
  while(1){
    ...
    /* if(i==1 && FIRST_GAP>gap_thd){
      // printf("First Gap %.4lf, using original search...\n",FIRST_GAP);
      timeout=time_left;
      init_best();
    }else{
      //printf("First Gap %.4lf, using dual bounds search...\n",FIRST_GAP);
      init_solution();
      }*/
    init_solution();
    localsearch(timeout);
  ...
  if(OPTIMAL || best_best_value==INT_MAX)
