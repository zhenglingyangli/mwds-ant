...
void constructInitPerturbed(double skip_rate){
	int v, v_n;
	sol_Cur_Cost = 0;
	sol_Best_Cost = 0;
	sol_Best_Size = 0;
	...
	Step = 0;
	for (v = 1; v <= v_Num; ++v) {
		if (v_State[v] == STATE::Deleted) continue;
		if (v_State[v] == STATE::Fixed || v_State[v] > 10) {
			fix_Num++;
			continue;
		}
	...
	checkSol();
	for(int i=0;i<USED(VEC_SOLUTION);i++){
		int vv=ITEM(VEC_SOLUTION,i);
		if (v_State[vv] == STATE::Fixed || v_State[vv] > 10) continue;
		if(rand()/(RAND_MAX+1.0) < skip_rate) continue;
		newAddVertexInit(vv);
	}
	while(Undmnd->size_ > 0){
		int und_v = Undmnd->arr_[rand() % Undmnd->size_];
		int best_v = 0;
		int best_score = INT_MIN;
		int best_cost = 1;
		for(int j = 0; j < v_Dmnd_size[und_v]; j++){
			v_n = v_Dmnd[und_v][j];
			if(sol_Cur->pos_[v_n] != -1) continue;
			int rn = compare(best_score, best_cost, v_Score[v_n], v_Cost[v_n]);
			if(rn < 0){
				best_v = v_n;
				best_score = v_Score[v_n];
				best_cost = v_Cost[v_n];
			} else if(rn == 0 && rand() % 2 == 0){
				best_v = v_n;
			}
		}
		if(best_v > 0)
			newAddVertexInit(best_v);
		else
			break;
	}
	eliminateRedundancy();
...
int getAddVRand()
{
	...
	for (int i = 0; i < v_Dmnd_size[v]; ++i)
	{
		...
		else if (rn == 0)
		{
			if (cc[v_aim] < cc[v_n])
				v_aim = v_n;
			else if (cc[v_aim] == cc[v_n]){
				if(AQ_ENABLED && AQ[v_n] > AQ[v_aim])
					v_aim = v_n;
				else if(v_Time_Stamp[v_aim] > v_Time_Stamp[v_n])
					v_aim = v_n;
			}
		}
	}
